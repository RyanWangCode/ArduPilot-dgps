 ...
 *
 *       Based on DCM code by Doug Weibel, Jordi Mu锟給z and Jose Julio. DIYDrones.com
 *
 ...
 *  We call the process of enforcing the orthogonality conditions 锟絩enormalization锟�.
 */
void
...
//    } else if (_flags.fly_forward && have_gps()) {
    ...
    // if (have_gps() && _gps.ground_course_cd()>0.01f)
    {
    //    if(_gps.last_fix_time_ms() != _gps_last_update)
        {
            ...
            */
            if (!_flags.have_initial_yaw || lastyaw_d<0.0001f)
					// yaw_deltat > 20
//					(_gps.ground_speed() >= 3*GPS_SPEED_MIN && fabsf(yaw_error_rad) >= 1.047f)) {
            ...
            if((lastyaw_d<0.0001f || fabs(yaw_error_rad)<0.5236f) && lastyaw_d!=gps_course_rad)
            {
                new_value = true;
            }

            lastyaw_d=gps_course_rad;
...
//            	yaw_error = 0;
//            }
//            else if (yaw_deltat > 20 ||(_gps.ground_speed() >= 3*GPS_SPEED_MIN && fabsf(yaw_error_rad) >= 0.7854f)) {
//                // reset DCM matrix based on current yaw
//                if(fabsf(gps_course_rad - yaw) < M_PI)
//            	{ygps_corr=yaw+(gps_course_rad-yaw)*ygps_gain;}
//                else if(yaw>gps_course_rad)
//                {
//                	ygps_corr=yaw+(M_2PI+gps_course_rad-yaw)*ygps_gain;
//                	if(ygps_corr>=M_2PI)
//                	{ygps_corr=ygps_corr-M_2PI;}
//                }
//                else
//                {
//                	ygps_corr=yaw+(-M_2PI+gps_course_rad-yaw)*ygps_gain;
